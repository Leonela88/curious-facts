...
export async function displayFact() {
    const factTextElement = document.getElementById('fact-text');
    const statusMessageElement = document.getElementById('status-message');

   
    if (!factTextElement || !statusMessageElement) return;
...
export function saveFavoriteFact() {
...
function init() {
    ...
    if (newFactButton) newFactButton.addEventListener('click', displayFact);
    if (saveFavButton) saveFavButton.addEventListener('click', saveFavoriteFact);
